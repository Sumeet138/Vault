import { NextRequest, NextResponse } from 'next/server';
import {
  getAssetById,
  updateAssetShares,
  addHolding,
  createTransaction,
} from '@/lib/mongodb/rwa';

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const {
      assetId,
      userId,
      quantity,
      transactionHash,
    } = body;
    
    // Validate input
    if (!assetId || !userId || !quantity || !transactionHash) {
      return NextResponse.json(
        {
          success: false,
          error: 'Missing required fields: assetId, userId, quantity, transactionHash',
        },
        { status: 400 }
      );
    }
    
    if (quantity <= 0) {
      return NextResponse.json(
        {
          success: false,
          error: 'Quantity must be greater than 0',
        },
        { status: 400 }
      );
    }
    
    // Get asset details
    const asset = await getAssetById(assetId);
    if (!asset) {
      return NextResponse.json(
        {
          success: false,
          error: 'Asset not found',
        },
        { status: 404 }
      );
    }
    
    // Check availability
    if (asset.availableShares < quantity) {
      return NextResponse.json(
        {
          success: false,
          error: `Only ${asset.availableShares} shares available`,
        },
        { status: 400 }
      );
    }
    
    // Calculate total price
    const totalPrice = asset.pricePerShare * quantity;
    
    // Create transaction record
    const transactionCreated = await createTransaction({
      assetId,
      buyerUserId: userId,
      quantity,
      totalPrice,
      status: 'COMPLETED',
      transactionHash,
    });
    
    if (!transactionCreated) {
      return NextResponse.json(
        {
          success: false,
          error: 'Failed to create transaction record',
        },
        { status: 500 }
      );
    }
    
    // Update asset shares
    const sharesUpdated = await updateAssetShares(assetId, quantity);
    if (!sharesUpdated) {
      return NextResponse.json(
        {
          success: false,
          error: 'Failed to update asset shares',
        },
        { status: 500 }
      );
    }
    
    // Add to user holdings
    const holdingAdded = await addHolding({
      userId,
      assetId,
      quantity,
      purchasePrice: asset.pricePerShare,
      purchaseDate: new Date(),
      transactionHash,
    });
    
    if (!holdingAdded) {
      return NextResponse.json(
        {
          success: false,
          error: 'Failed to add holding',
        },
        { status: 500 }
      );
    }
    
    return NextResponse.json({
      success: true,
      data: {
        assetId,
        quantity,
        totalPrice,
        transactionHash,
      },
    });
  } catch (error) {
    console.error('Error processing RWA purchase:', error);
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to process purchase',
      },
      { status: 500 }
    );
  }
}

